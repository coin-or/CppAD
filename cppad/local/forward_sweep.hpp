...
zero order ( \a q = 0 ) versions of the VecAD operators LdpOp and LdvOp 
...
This must be greater than or equal \a q + 1.

\param taylor
\b Input: For j = 1 , ... , \a n, and for k = 0 , ... , \a q,
\a taylor [ j * J + k ]
...
\n
\n
\b Output: For i = \a n + 1, ... , \a numvar - 1, and for k = 0 , ... , \a q,
\a taylor [ i * J + k ]
...
# endif
			...
			for(ell = 1; ell < user_q1; ell++)
				user_tx[user_j * user_q1 + ell] = Base(0);
			++user_j;
			if( user_j == user_n )
				user_state = user_ret;
			break;
			...
			for(ell = 0; ell < user_q1; ell++)
				user_tx[user_j * user_q1 + ell] = taylor[ arg[0] * J + ell];
			++user_j;
			if( user_j == user_n )
				user_state = user_ret;
			break;
			...
			for(ell = 1; ell < p; ell++)
				user_ty[user_i * user_q1 + ell] = Base(0);
			user_i++;
			if( user_i == user_m )
				user_state = user_end;
			break;
			...
			for(ell = 0; ell < p; ell++)
				user_ty[user_i * user_q1 + ell] = taylor[ i_var * J + ell];
			user_i++;
			if( user_i == user_m )
				user_state = user_end;
			break;
