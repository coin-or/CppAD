// $Id$
# ifndef CPPAD_FOR_SPARSE_HES_HPP
# define CPPAD_FOR_SPARSE_HES_HPP
...
$head Syntax$$
$icode%h% = %f%.ForSparseHes(%q%, %s%)
%$$
$icode%h% = %f%.ForSparseHes(%q%, %s%, %transpose%)%$$
...
{	size_t n = Domain();
	...
	CPPAD_ASSERT_KNOWN(
		...
		"ForSparseHes: previous stored call to ForSparseJac did not "
