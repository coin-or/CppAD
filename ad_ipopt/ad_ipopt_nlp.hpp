# ifndef CPPAD_AD_IPOPT_NLP_INCLUDED
# define CPPAD_AD_IPOPT_NLP_INCLUDED
/* --------------------------------------------------------------------------
...
%$$
$codei%# ad_ipopt_solution %solution%;
%$$
$codei%ad_ipopt_nlp %cppad_nlp%(
	%n%, %m%, %x_i%, %x_l%, %x_u%, %g_l%, %g_u%, &%fg_info%, &%solution%
)%$$

$head Purpose$$
The class $code ad_ipopt_nlp$$ is used to solve nonlinear programming
...
$codei ad_ipopt_nlp.hpp$$ include file to be a 
...
base class $code ad_ipopt_fg_info$$.
...
$codei%
	virtual size_t ad_ipopt_fg_info::number_functions(void)
%$$
...
the $cref/representation/ad_ipopt_nlp/fg(x)/Representation/$$ above.
$pre

$$
The $code ad_ipopt_fg_info$$ implementation of this function
...
$codei%
virtual ADVector ad_ipopt_fg_info::eval_r(size_t %k%, const ADVector& %u%) = 0;
%$$
...
used in the $cref/representation/ad_ipopt_nlp/fg(x)/Representation/$$
for $latex fg(x)$$.
...
This member function has the prototype
$codei%
	virtual bool ad_ipopt_fg_info::retape(size_t %k%)
%$$
If $icode k$$ in $latex \{0 , \ldots , K-1 \}$$ has type $code size_t$$,
...
$code ad_ipopt_nlp$$ will retape the operation sequence 
corresponding to $latex r_k (u)$$ for
every value of $icode u$$. 
An $code ad_ipopt_nlp$$ object
...
This member function has prototype
$codei%
	virtual size_t ad_ipopt_fg_info::domain_size(size_t %k%)
%$$
If $icode k$$ in $latex \{0 , \ldots , K-1 \}$$ has type $code size_t$$,
...
$$
The $code ad_ipopt_h_base$$ implementation of this function
corresponds to the simple representation mentioned above; i.e.,
$latex q = n$$.
...
This member function has prototype
$codei%
	virtual size_t ad_ipopt_fg_info::range_size(size_t %k%)
%$$
If $icode k$$ in $latex \{0 , \ldots , K-1 \}$$ has type $code size_t$$,
...
This member function has prototype
$codei%
	virtual size_t ad_ipopt_fg_info::number_terms(size_t %k%)
%$$
If $icode k$$ in $latex \{0 , \ldots , K-1 \}$$ has type $code size_t$$,
...
This member function has prototype
$codei%
	virtual void ad_ipopt_fg_info::index(
...
in the $cref/representation/ad_ipopt_nlp/fg(x)/Representation/$$ above.
The argument 
$icode%
	J
%$$ is a $cref/SimpleVector/$$ with elements
...
$codei%
	ad_ipopt_solution::solution_status %solution%.status
%$$
...
$rnext
not_defined $cnext
The optimizer did not return a final status to this $code ad_ipopt_nlp$$
object.
$rnext
unknown $cnext
...
$head Example$$
The file 
$cref/ad_ipopt_simple.cpp/$$ is an example and test of $code ad_ipopt_nlp$$.
that uses the 
$cref/simple representation/ad_ipopt_nlp/Simple Representation/$$.
The file
$cref/ad_ipopt_ode.cpp/$$ is a more complex example that optimizes
...
*/
class ad_ipopt_fg_info
{
	friend class ad_ipopt_nlp;
private:
	size_t n_;
	size_t m_;
...
*/
class ad_ipopt_nlp : public Ipopt::TNLP
{
...
private:
	...
	*/
	...
	ad_ipopt_fg_info* const      fg_info_;
	// object for storing final solution results
	ad_ipopt_solution* const     solution_;
	...
	*/
	...
	ad_ipopt_nlp(const ad_ipopt_nlp&);
	ad_ipopt_nlp& operator=(const ad_ipopt_nlp&);
