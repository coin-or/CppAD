...
# ---------------------------------------------------------------------
list="AUTHORS"
svn cat AUTHORS | sed > bin/only_date.$$ \
	-e "s/, [0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\} *,/, $yyyy_mm_dd,/"
check_difference AUTHORS
# ---------------------------------------------------------------------
list="$list configure"
svn cat configure | sed > bin/only_date.$$ \
...
#
svn cat cppad/config.h | sed > bin/only_date.$$ \
	-e "s/CppAD [0-9]\{8\}[.0-9]*/CppAD $version/g" \
	-e "s/VERSION \"[0-9]\{8\}[.0-9]*\"/VERSION \"$version\"/g"
...
svn cat cppad/configure.hpp | sed > bin/only_date.$$ \
	-e "s/CppAD [0-9]\{8\}[.0-9]*/CppAD $version/g"
