...
test_more.cpp: fix warnings generated by MS compiler.
test_more.vcproj: add fun_check.cpp to list of source files.
faq.omh: remove Base type restriction.
cond_exp.hpp: add comment as to why two template parameters.
ordered.hpp: change call by value to const call by reference.
user_ad.hpp: include Base type requirements below here.
abs.hpp: change call by value to const call by reference.
pow.hpp: change call by value to const call by reference.
erf.hpp: change call by value to const call by reference.
std_math_unary.hpp: change to call by reference and fix description of tan.
" 
add_list="
