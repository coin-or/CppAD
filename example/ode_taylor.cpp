...
The first level of taping is used to tape the solution of an 
ordinary differential equation.
This solution is then differentiated with respect to a parameter vector.
The second level of taping is used to take derivatives during
...
private:
	...
	CppADvector< ADdouble > x_; 
public:
	// constructor
	Ode( CppADvector< ADdouble > x) : x_(x)
	{ }
	// the function g(y) is evaluated with two levels of taping
	CppADvector< ADDdouble > operator()
	( const CppADvector< ADDdouble > &y) const
	{	size_t n = y.size();
		CppADvector< ADDdouble > g(n);
		size_t i;
		g[0] = x_[0];
		for(i = 1; i < n; i++)
			g[i] = x_[i] * y[i-1];
...
CppADvector < ADdouble > taylor_ode(
	...
	CppADvector< ADdouble > &y_ini  )  // y(t) at the initial time
{
	...
	CppADvector< ADDdouble >   Y(n), Z(n);

	// y, y^{(k)} , z^{(k)}, and y^{(k+1)}
	CppADvector< ADdouble >  y(n), y_k(n), z_k(n), y_kp(n);
...
{	// initialize the return value as true	
	...
	// check Jacobian of h evaluated at a
