...
$section Template Function that Evaluates Example Algorithm$$

$head Syntax$$
$icode%y% = algorithm(%x%, %y%)%$$
...
$head y$$
This output vector
is the result of applying the algorithm with inputs $icode x$$ and $icode p$$.
We use $latex m$$ to denote the size of this vector.
The algorithm evaluates a function
$latex \[
    f : \B{R}^n \times \B{R}^\ell  \rightarrow \B{R}^m
\] $$.
...
VectorFloat algorithm(const VectorFloat& x, const VectorFloat& p)
// END_PROTOTYPE
{   size_t nx = x.size();
    size_t np = p.size();
    size_t ny = nx + np;
