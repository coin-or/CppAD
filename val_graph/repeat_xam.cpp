...
/*
{xrst_begin val_repeat_xam.cpp dev}
...
{xrst_end val_repeat_xam.cpp}
*/
// BEGIN_C++
bool repeat_xam(void)
{  bool ok = true;
   //
   ...
   op_arg[0]   = 0; // x[0]
   op_arg[1]   = five;
   addr_t sum  = tape.record_op(add_op_enum, op_arg); // x[0] + 5.0
   //
   // tape, sum
   addr_t six = tape.record_con_op(6.0);
   op_arg[0]  = sum; // x[0] + 5.0
   op_arg[1]  = six;
   sum        = tape.record_op(add_op_enum, op_arg); // x[0] + 5.0 + 6.0
   //
   // tape, sum
   addr_t another_five = tape.record_con_op(5.0);
   op_arg[0]  = sum; // x[0] + 5.0 + 6.0 + 5.0
   op_arg[1]  = another_five;
   sum        = tape.record_op(add_op_enum, op_arg); // x[0] + 5.0 + 6.0 + 5.0
   //
   // set_dep
