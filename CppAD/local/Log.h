# ifndef CppADLogIncluded
# define CppADLogIncluded
...
$bold Syntax$$ 
$cnext 
$syntax%AD<%Base%> log (const AD<%Base%> &%x%)%$$
$tend
...
$syntax%
	log(%x%)
%$$
where $code log$$ has the same interpretation as
for the corresponding $xref/glossary/Base Type/base type/$$.


$head Base Type Requirement$$
A definition for the $code log$$ function
...
If the $code log$$ function is used with any other base type, 
...
CppADStandardMathFun(log)

template <class Base>
AD<Base> AD<Base>::Log (void) const
{	using CppAD::log;
...
template <class Base>
inline AD<Base> log(const AD<Base> &x)
{	return x.Log(); }

template <class Base>
inline AD<Base> log(const VecADelem<Base> &x)
{	return log( x.ADBase() ); }
